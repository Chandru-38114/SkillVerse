...
@router.post("/dev-prep-test")
def dev_prep_test(
    db: Session = Depends(get_db),
    current_user: models.User = Depends(auth.get_current_user)
):
    if os.getenv("DEV_MODE", "").lower() != "true":
        raise HTTPException(status_code=403, detail="Forbidden: Not in dev mode")

    skill_name = "Certificate Testing"
    ...
    us.level = "Advanced"
    us.badge = "Gold"
    us.sessions_completed = max(us.sessions_completed, 5)
    
    db.commit()
    
    return {"detail": "Test skill prepared successfully."}
